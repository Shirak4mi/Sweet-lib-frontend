					...
					<div className="hidden lg:flex items-center space-x-1">
						{navItems.map((item, index) => (
							<div key={item.label} className="relative group dropdown-container">
								...
								{/* Dropdown Menu */}
								{item.children && <DropdownMenu item={item} isOpen={activeDropdown === item.label} onClose={closeDropdown} />}
							</div>
						))}
				...
				{mobileMenuOpen && (
					<motion.div
						...
						<div className="container mx-auto px-4 py-4">
							{navItems.map((item, index) => (
								<div key={item.label}>
									<motion.div animate={{ opacity: 1, x: 0 }} initial={{ opacity: 0, x: -20 }} transition={{ duration: 0.3, delay: index * 0.1 }}>
										{item.children ? (
											<div>
												<button
													onClick={(e) => toggleDropdown(item.label, e)}
							...
							<motion.div
								...
								<div className="flex items-center justify-between">
									...
									<Button variant="outline" className="w-full bg-white text-blue-800 border-white hover:bg-blue-50" onClick={closeMobileMenu}>
										LOGIN
									</Button>
								</div>
							</motion.div>
